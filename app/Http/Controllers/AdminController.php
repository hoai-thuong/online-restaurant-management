...
class AdminController extends Controller
{
    ...
    public function order_complete()
    {
        ...
        $isAdmin = $user->usertype == 1; // Giả sử 1 là vai trò admin, bạn cần điều chỉnh theo vai trò của bạn
    
        if ($isAdmin) {
            // Nếu là admin, lấy tất cả đơn hàng đã hoàn thành
            ...
            // Nếu là nhân viên giao hàng, lấy các đơn hàng đã hoàn thành và phân công cho nhân viên đó
            $orders = DB::table('carts')
                        ->where('product_order', 'delivery')
                        ->where('delivery_staff_id', $user->id) // Lọc theo ID của nhân viên giao hàng
                        ->groupBy('invoice_no')
                        ->orderBy('id', 'desc') // Sắp xếp theo cột id giảm dần
                        ->get();
        }
    ...
    public function invoice_approve(Request $req,$id)
    {
       ...
       // return $req->time;
        ...
        $deliveryStaffId = $req->input('delivery_staff');


        // dd($deliveryStaffId);
        if ($deliveryStaffId) {
            $data['delivery_staff_id'] = $deliveryStaffId;
        }
    ...
    public function invoice_details($id)
    {
       ...
       $charges=DB::table('charges')->get();
        ...
        $deliveryStaff = DB::table('users')->where('usertype', 2)->get();
        // dd($deliveryStaff);
        return view('admin.invoice_details',compact('total_price','extra_charge','total_extra_charge','discount_price','wihout_discount_price','products', 'deliveryStaff'));
    ...
    public function orders_process()
    {
        ...
        if ($isAdmin) {
            // Nếu là admin, lấy tất cả đơn hàng đã được phê duyệt
            $orders = DB::table('carts')
            ...
            // Nếu là nhân viên giao hàng, lấy các đơn hàng đã được phân công cho nhân viên đó
            $orders = DB::table('carts')
                        ->where('product_order', 'approve')
                        ->where('delivery_staff_id', $user->id) // Lọc theo ID của nhân viên giao hàng
                        ->groupBy('invoice_no')
    ...
    public function orders_cancel()
    {
        ...
        if ($isAdmin) {
            // Nếu là admin, lấy tất cả đơn hàng bị hủy
            $orders = DB::table('carts')
                        ->where('product_order', 'cancel')
                        ->groupBy('invoice_no')
                        ->get();
        } else {
            // Nếu là nhân viên giao hàng, lấy các đơn hàng bị hủy và phân công cho nhân viên đó
            $orders = DB::table('carts')
                        ->where('product_order', 'cancel')
                        ->where('delivery_staff_id', $user->id) // Lọc theo ID của nhân viên giao hàng
                        ->groupBy('invoice_no')
